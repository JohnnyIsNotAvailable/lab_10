...
def game_over():
    # cur.execute(
    #     "INSERT INTO scores (username) VALUES (%s)",
    #     (name,)
    # )
    # cur.execute("SELECT user_id FROM scores WHERE username = %s", (name,))
    # user_id = cur.fetchone()[0]
    cur.execute(
        'INSERT INTO scores(username, score, level) VALUES (%s, %s, %s);',
        (name, score, level)
    )
    conn.commit()
    cur.close()
